 ...
 * Created: 2021-05-12
 * Description: Sets ECR tags to Immutable
...
module.exports.run = (event) => new Promise(async (resolve) => {
    ...
    if (event.playbook.type === 'schedule') {
        ...
        var url = `api/ecr/repositories/${event.accountDetails.accountId}?RecordStatus=Active&scanOnPush=false&fields=imageTagMutability,Region,repositoryName`;
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            await helper.awsApiCall(regionCredential, 'ECR', 'putImageTagMutability', { repositoryName: data[i].repositoryName, imageTagMutability: 'IMMUTABLE' });
        }
        ...
        if (item.imageScanningConfiguration.imageTagMutability === "MUTABLE") {
            // enabling scan on push
            await helper.awsApiCall(credential, 'ECR', 'putImageTagMutability', { repositoryName: item.repositoryName, imageTagMutability: 'IMMUTABLE' });
