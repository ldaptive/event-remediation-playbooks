 ...
 * Created: 2021-05-12
 * Description: Disable access keys that have not been used for specified days
...
module.exports.run = (event) => new Promise(async (resolve) => {
    ...
    var url = `api/iam/accesskeys/${event.accountDetails.accountId}?RecordStatus=Active&Status=Active&LastUsedDate[lt]=${pastDate}&fields=UserName,AccessKeyId,LastUsedDate`;
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        await helper.awsApiCall(credential, 'IAM', 'updateAccessKey', { AccessKeyId: data[i].AccessKeyId, Status: 'Inactive' });
    }
